...
- **Network resilience**: supabase client created with `postgrest_client_timeout=8`/`storage_client_timeout=8` (never hangs forever on a blackholed connection). `set_command_status()` failures on network errors are enqueued to `pending_ops.db` (kind `command_status`) and retried by the worker. `recover_stale_commands()` at startup marks any leftover `running` commands as failed ("Bi gian doan khi Pi khoi dong lai"). `fina.service` has `WatchdogSec=30` -- a dedicated daemon thread pings `WATCHDOG=1` via `systemd-notify` every 10s (unconditional, NOT tied to the main loop, so long commands are never killed); if the whole process freezes >30s, systemd kills and restarts it (no SSH needed). Frontend polls must survive network blips (keep polling, show "Mất kết nối...").
