...
- Main flow (main.py) is **command-driven, not continuous recognition**: idle = TFT shows "WELCOME TO SMART LOCK" + camera closed. Every `POLL_INTERVAL=3s` it fetches one pending `device_commands` row via `fetch_pending_command()` (any command for this device) and executes it with the camera open: `start_register_face` (payload `{face_name}` -> registration flow), `start_checkin`/`start_checkout` (payload `{}` -> verify face, record `access_logs` with `access_type`, open relay 3s on grant, `VERIFY_TIMEOUT=45s`), `restart_service` (payload `{}` -> marks done then `sys.exit(0)`; systemd `Restart=always` starts it back up -- no sudo needed). Unknown commands -> failed. Heartbeat every 15s, profile sync every 60s, systemd watchdog ping every 10s. Grant when cosine >= 0.40; faces smaller than `MIN_FACE_SIZE=60`px unknown; `unknown` when no profile or score < 0.15, `denied` otherwise. Matching takes the **max score across all templates** of a person. YuNet detection threshold is 0.65 (both main.py and registration).
- **Network resilience**: supabase client created with `postgrest_client_timeout=8`/`storage_client_timeout=8` (never hangs forever on a blackholed connection). `set_command_status()` failures on network errors are enqueued to `pending_ops.db` (kind `command_status`) and retried by the worker. `recover_stale_commands()` at startup marks any leftover `running` commands as failed ("Bi gian doan khi Pi khoi dong lai"). `fina.service` has `WatchdogSec=30` -- main.py pings `WATCHDOG=1` via `systemd-notify` every 10s; if the main loop blocks >30s for any reason, systemd kills and restarts it automatically (no SSH needed). Frontend polls must survive network blips (keep polling, show "Mất kết nối...").
